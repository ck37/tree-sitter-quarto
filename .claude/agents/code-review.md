...
**Codebase**: tree-sitter-quarto parser for Quarto Markdown, used in Zed editor extension
**Threat Profile**: Medium-High (library for developers, untrusted input, editor context, supply chain impact)
**Primary Languages**: JavaScript (grammar/tooling), C11 (external scanner), Tree-sitter DSL, Scheme (queries)
**Critical Dependencies**: tree-sitter CLI 0.25.10 (version MUST match headers)

**Stack-Specific Security Priorities:**

1. **C Scanner Security (HIGH PRIORITY)**
   - Buffer bounds checking (uses 7-byte serialization buffer)
   - Memory safety (proper calloc/free lifecycle)
   - Null pointer checks before dereference
   - Integer overflow prevention (uint32_t for fence_length)
   - State corruption on deserialization (fallback handling)

2. **Parser Robustness (HIGH PRIORITY)**
   - Maliciously crafted documents (buffer overflows, infinite loops, crashes)
   - DoS vectors (excessive memory, CPU consumption)
   - Graceful degradation on malformed input
   - Editor responsiveness (no hangs, reasonable memory bounds)
   - No quadratic parsing behavior

3. **Tree-sitter Critical Rules (BLOCKING)**
   - **NEVER manually edit src/parser.c** - breaks ABI, must always regenerate
   - CLI version MUST match (0.25.10) - mismatches cause build failures
   - Commit parser.c and headers together
   - External scanner state serialization must be version-safe

4. **Code Conventions (BLOCKING)**
   - Lowercase filenames (except README.md, LICENSE, etc.)
   - **NO** Claude co-signing in commits (no "Generated with [Claude Code]", no "Co-Authored-By: Claude")
   - JavaScript: 2-space indent, camelCase, ES6+
   - C: 4-space indent, snake_case, C11 standard
   - Grammar: JSDoc comments with spec references, source tracking for modifications

5. **Performance Requirements (HIGH PRIORITY)**
   - Build time baselines maintained (warn on >20% regression)
   - WASM size <500KB uncompressed
   - Parse performance suitable for real-time editor use
   - No blocking operations in hot paths

6. **Testing Standards (MEDIUM PRIORITY)**
   - Every feature needs corpus test cases
   - OpenSpec proposals required before implementation
   - Real-world validation against Quarto corpus (targeting 90% success)

7. **Supply Chain Hygiene (MEDIUM PRIORITY)**
   - Dependency pinning enforced (tree-sitter CLI locked to 0.25.10)
   - Regular npm audit compliance
   - No dynamic Makefile generation

**Common Vulnerability Patterns to Watch:**
- Command injection in execSync() calls (validate file paths)
- Path traversal in benchmark/validation scripts
- Buffer overflows in scanner serialization
- State machine edge cases in external scanner
- Regex catastrophic backtracking (low risk - hardcoded patterns)
